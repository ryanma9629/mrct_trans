...
from llm import APIError, LLMService, SupportedProvider
...
class DictionaryMatcher:
    ...
    def _load_dictionary(self, csv_file: Union[str, Path]) -> None:
        ...
        try:
            ...
            logger.info(
                f"Loaded {len(self.en_to_cn)} EN→CN dictionary entries from {csv_path}"
            )
    ...
    def _find_text_matches(
        self, text: str, dictionary: Dict[str, str], is_english: bool
    ) -> List[Tuple[str, str, int, int]]:
        ...
        for term in sorted_terms:
            try:
                ...
                for match in pattern.finditer(search_text):
                    ...
                    if not self._overlaps_with_used_positions(
    ...
    def _overlaps_with_used_positions(
        self, start: int, end: int, used_positions: set
    ) -> bool:
...
class TranslationService:
    ...
    def __init__(
        self,
        dictionary_file: Union[str, Path] = "QS-TB.csv",
    ...
    def _init_dictionary_matcher(
        self, dictionary_file: Union[str, Path]
    ) -> DictionaryMatcher:
    ...
    def _check_context_data_path(self) -> bool:
        ...
        if not os.path.isdir(self.context_data_path):
            logger.warning(
                f"Context data directory not found: '{self.context_data_path}'."
            )
            logger.info(
                "Context retrieval will be disabled. Run 'python load_pdfs.py' to create it."
            )
            return False

        if not any(
            fname.endswith(".txt") for fname in os.listdir(self.context_data_path)
        ):
            logger.warning(
                f"Context data directory '{self.context_data_path}' is empty."
            )
            logger.info(
                "Context retrieval will be disabled. Run 'python load_pdfs.py' to populate it."
            )
            return False

        logger.info(
            f"Context data found at '{self.context_data_path}'. Context retrieval is enabled."
        )
        return True
    ...
    def _retrieve_text_context(
        ...
        # Check cache first
        cached_result = cache_manager.get_context(
            text, chapter_number, similarity_threshold
        )
        ...
        if similarity >= similarity_threshold:
            logger.info(
                f"Using context with similarity {similarity:.4f} (threshold: {similarity_threshold})"
            )
            self._log_context_details(context, chapter_number)
            result = (context, similarity)
        else:
            logger.warning(
                f"Match similarity {similarity:.4f} below threshold {similarity_threshold}"
            )
    ...
    def _validate_context_retrieval(self, chapter_number: int) -> bool:
        ...
        if not os.path.exists(file_path):
            logger.warning(
                f"Context file not found for Chapter {chapter_number}: {file_path}"
            )
            return False
    ...
    def _find_best_context_match(
        self, text: str, chapter_text: str
    ) -> Tuple[float, str]:
        ...
        matcher = SequenceMatcher(
            None, normalized_input, normalized_chapter, autojunk=False
        )
        match = matcher.find_longest_match(
            0, len(normalized_input), 0, len(normalized_chapter)
        )
    ...
    def _extract_context_window(
        self, chapter_text: str, match, normalized_chapter: str
    ) -> str:
        ...
        start_idx, end_idx = self._map_to_original_text(
            matched_block, chapter_text, match.b, match.size
        )
    ...
    def _prepare_dictionary_context(
        self, dictionary_matches: Optional[List[Tuple[str, str, int, int]]]
    ) -> str:
    ...
    def _prepare_system_prompt(
        ...
        direction = (
            "from English to Chinese" if is_english else "from Chinese to English"
        )
        ...
        logger.debug(
            f"Prepared prompt with context: {bool(context)}, dict terms: {len(dictionary_matches) if dictionary_matches else 0}"
        )
        return full_prompt
    ...
    def _build_base_translation_prompt(self, direction: str) -> str:
        ...
        return (
            ...
            f"- Do not translate personal names\n"
    ...
    def _build_context_prompt(self, context: str) -> str:
        ...
        return (
            f"\n\nRELEVANT CONTEXT: The following text provides context from the same chapter to help you understand "
    ...
    async def translate(
        ...
        str, List[Tuple[str, str, int, int]], Optional[Dict[str, Union[str, float]]]
    ]:
        ...
        try:
            ...
            context_info = self._retrieve_context_if_requested(
                text, use_context, chapter_number
            )

            self._log_translation_request(
                text, llm_provider, use_context, chapter_number, matches
            )

            # Perform translation
            translated = await self._perform_translation(
                text, is_english, matches, context_info, llm_provider, api_token, model
            )

            self._log_translation_completion(
                text, translated, context_info, matches, chapter_number, use_context
            )
    ...
    def _retrieve_context_if_requested(
        self, text: str, use_context: bool, chapter_number: Optional[int]
        ...
        if not self.context_enabled:
            logger.warning(
                "Context requested but not available. Proceeding without context."
            )
            return None
        ...
        if chapter_number is not None:
            ...
            if context_result:
                ...
                logger.warning(
                    f"No suitable context found for Chapter {chapter_number}"
                )
    ...
    async def _perform_translation(
        ...
        # Check cache first
        cached_translation = cache_manager.get_translation(
            text, llm_provider, model_name, system_prompt_hash
        )
        ...
        cache_manager.cache_translation(
            text, llm_provider, model_name, system_prompt_hash, translated
        )
        logger.debug(f"Translation cached for provider {llm_provider}")

        return translated

    def _log_translation_request(
        self,
        text: str,
        llm_provider: str,
        use_context: bool,
        ...
        logger.info(
            f"Translation request - {len(text)} chars, provider: {llm_provider}, "
            f"context: {use_context}, chapter: {chapter_number}, matches: {len(matches)}"
        )

    def _log_translation_completion(
        self,
        text: str,
        translated: str,
        context_info: Optional[Dict],
        ...
        logger.info(
            f"Translation completed - Input: {len(text)} chars, Output: {len(translated)} chars, "
    ...
    def get_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "tokens_available": {
                SupportedProvider.CHATGPT.value: bool(
                    os.getenv("OPENAI_API_KEY", "").strip()
                ),
                SupportedProvider.CHATGPT_AZURE.value: bool(
                    os.getenv("AZURE_OPENAI_API_KEY", "").strip()
                ),
                SupportedProvider.DEEPSEEK.value: bool(
                    os.getenv("DEEPSEEK_API_KEY", "").strip()
                ),
                SupportedProvider.QWEN.value: bool(
    ...
    async def translate_stream(
        ...
        try:
            ...
            system_prompt = self._prepare_system_prompt(
                is_english, matches, context_text
            )
